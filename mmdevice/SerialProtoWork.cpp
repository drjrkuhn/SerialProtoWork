...
const char* g_DeviceNameSerialProtoWorkSwitch = "SerialProtoWork-Switch";
const char* g_DeviceNameSerialProtoWorkShutter = "SerialProtoWork-Shutter";
const char* g_DeviceNameSerialProtoWorkDA1 = "SerialProtoWork-DAC1";
const char* g_DeviceNameSerialProtoWorkDA2 = "SerialProtoWork-DAC2";
const char* g_DeviceNameSerialProtoWorkInput = "SerialProtoWork-Input";
...
MODULE_API void InitializeModuleData()
{
   ...
   RegisterDevice(g_DeviceNameSerialProtoWorkSwitch, MM::StateDevice, "Digital out 8-bit");
   RegisterDevice(g_DeviceNameSerialProtoWorkShutter, MM::ShutterDevice, "Shutter");
   RegisterDevice(g_DeviceNameSerialProtoWorkDA1, MM::SignalIODevice, "DAC channel 1");
   RegisterDevice(g_DeviceNameSerialProtoWorkDA2, MM::SignalIODevice, "DAC channel 2");
   RegisterDevice(g_DeviceNameSerialProtoWorkInput, MM::GenericDevice, "ADC");
}
...
MODULE_API MM::Device* CreateDevice(const char* deviceName)
{
   ...
   else if (strcmp(deviceName, g_DeviceNameSerialProtoWorkSwitch) == 0)
   {
      return new CSerialProtoWorkSwitch;
   }
   else if (strcmp(deviceName, g_DeviceNameSerialProtoWorkShutter) == 0)
   {
      return new CSerialProtoWorkShutter;
   }
   else if (strcmp(deviceName, g_DeviceNameSerialProtoWorkDA1) == 0)
   {
      return new CSerialProtoWorkDA(1); // channel 1
   }
   else if (strcmp(deviceName, g_DeviceNameSerialProtoWorkDA2) == 0)
   {
      return new CSerialProtoWorkDA(2); // channel 2
   }
   else if (strcmp(deviceName, g_DeviceNameSerialProtoWorkInput) == 0)
   {
      return new CSerialProtoWorkInput;
   }
...
int CSerialProtoWorkHub::DetectInstalledDevices()
{
   if (MM::CanCommunicate == DetectDevice()) 
   {
      ...
      peripherals.clear();
      peripherals.push_back(g_DeviceNameSerialProtoWorkSwitch);
      peripherals.push_back(g_DeviceNameSerialProtoWorkShutter);
      peripherals.push_back(g_DeviceNameSerialProtoWorkInput);
      peripherals.push_back(g_DeviceNameSerialProtoWorkDA1);
      peripherals.push_back(g_DeviceNameSerialProtoWorkDA2);
...
///////////////////////////////////////////////////////////////////////////////
// CSerialProtoWorkSwitch implementation
// ~~~~~~~~~~~~~~~~~~~~~~~~~~

CSerialProtoWorkSwitch::CSerialProtoWorkSwitch() : 
   nrPatternsUsed_(0),
   currentDelay_(0),
   sequenceOn_(false),
   blanking_(false),
   ...
   busy_(false)
{
   ...
   SetErrorText(ERR_COMMUNICATION, "Error in communication with SerialProtoWork board");
   SetErrorText(ERR_NO_PORT_SET, "Hub Device not found.  The SerialProtoWork Hub device is needed to create this device");
   ...
   // Description
   int ret = CreateProperty(MM::g_Keyword_Description, "SerialProtoWork digital output driver", MM::String, true);
   assert(DEVICE_OK == ret);

   // Name
   ret = CreateProperty(MM::g_Keyword_Name, g_DeviceNameSerialProtoWorkSwitch, MM::String, true);
   assert(DEVICE_OK == ret);
...
void CSerialProtoWorkSwitch::GetName(char* name) const
{
   CDeviceUtils::CopyLimitedString(name, g_DeviceNameSerialProtoWorkSwitch);
}


int CSerialProtoWorkSwitch::Initialize()
{
   ...
   // State
   // -----
   CPropertyAction* pAct = new CPropertyAction (this, &CSerialProtoWorkSwitch::OnState);
   ...
   pAct = new CPropertyAction(this, &CSerialProtoWorkSwitch::OnSequence);
   ...
   pAct = new CPropertyAction(this, &CSerialProtoWorkSwitch::OnBlanking);
   ...
   pAct = new CPropertyAction(this, &CSerialProtoWorkSwitch::OnBlankingTriggerDirection);
   ...
   pAct = new CPropertyAction(this, &CSerialProtoWorkSwitch::OnStartTimedOutput);
   ...
   // This delay will be transferred to the SerialProtoWork using the Get and SetPattern commands
   pAct = new CPropertyAction(this, &CSerialProtoWorkSwitch::OnDelay);
   ...
   pAct = new CPropertyAction(this, &CSerialProtoWorkSwitch::OnRepeatTimedPattern);
...
int CSerialProtoWorkSwitch::Shutdown()
{
   initialized_ = false;
   return DEVICE_OK;
}

int CSerialProtoWorkSwitch::WriteToPort(long value)
{
   CSerialProtoWorkHub* hub = static_cast<CSerialProtoWorkHub*>(GetParentHub());
   if (!hub || !hub->IsPortAvailable()) {
      return ERR_NO_PORT_SET;
...
int CSerialProtoWorkSwitch::LoadSequence(unsigned size, unsigned char* seq)
{
   CSerialProtoWorkHub* hub = static_cast<CSerialProtoWorkHub*>(GetParentHub());
   if (!hub || !hub->IsPortAvailable())
      return ERR_NO_PORT_SET;
...
int CSerialProtoWorkSwitch::OnState(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   CSerialProtoWorkHub* hub = static_cast<CSerialProtoWorkHub*>(GetParentHub());
   if (!hub || !hub->IsPortAvailable())
      return ERR_NO_PORT_SET;
...
int CSerialProtoWorkSwitch::OnSequence(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet)
   {
      if (sequenceOn_)
...
int CSerialProtoWorkSwitch::OnStartTimedOutput(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   CSerialProtoWorkHub* hub = static_cast<CSerialProtoWorkHub*>(GetParentHub());
   if (!hub || !hub->IsPortAvailable())
      return ERR_NO_PORT_SET;
...
int CSerialProtoWorkSwitch::OnBlanking(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   CSerialProtoWorkHub* hub = static_cast<CSerialProtoWorkHub*>(GetParentHub());
   if (!hub || !hub->IsPortAvailable())
      return ERR_NO_PORT_SET;
...
int CSerialProtoWorkSwitch::OnBlankingTriggerDirection(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   CSerialProtoWorkHub* hub = static_cast<CSerialProtoWorkHub*>(GetParentHub());
   if (!hub || !hub->IsPortAvailable())
      return ERR_NO_PORT_SET;
...
int CSerialProtoWorkSwitch::OnDelay(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet) {
...
int CSerialProtoWorkSwitch::OnRepeatTimedPattern(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   CSerialProtoWorkHub* hub = static_cast<CSerialProtoWorkHub*>(GetParentHub());
   if (!hub || !hub->IsPortAvailable())
      return ERR_NO_PORT_SET;
...
///////////////////////////////////////////////////////////////////////////////
// CSerialProtoWorkDA implementation
// ~~~~~~~~~~~~~~~~~~~~~~

CSerialProtoWorkDA::CSerialProtoWorkDA(int channel) :
      busy_(false), 
      minV_(0.0), 
      maxV_(5.0), 
      volts_(0.0),
      ...
      gateOpen_(true)
{
   ...
   CPropertyAction* pAct = new CPropertyAction(this, &CSerialProtoWorkDA::OnChannel);
   ...
   */
   ...
   CPropertyAction* pAct = new CPropertyAction(this, &CSerialProtoWorkDA::OnMaxVolt);
   CreateProperty("MaxVolt", "5.0", MM::Float, false, pAct, true);

   name_ = channel_ == 1 ? g_DeviceNameSerialProtoWorkDA1 : g_DeviceNameSerialProtoWorkDA2;

   // Description
   int nRet = CreateProperty(MM::g_Keyword_Description, "SerialProtoWork DAC driver", MM::String, true);
...
CSerialProtoWorkDA::~CSerialProtoWorkDA()
{
   Shutdown();
}

void CSerialProtoWorkDA::GetName(char* name) const
{
   CDeviceUtils::CopyLimitedString(name, name_.c_str());
}


int CSerialProtoWorkDA::Initialize()
{
   CSerialProtoWorkHub* hub = static_cast<CSerialProtoWorkHub*>(GetParentHub());
   if (!hub || !hub->IsPortAvailable()) {
      return ERR_NO_PORT_SET;
   ...
   // State
   // -----
   CPropertyAction* pAct = new CPropertyAction (this, &CSerialProtoWorkDA::OnVolts);
...
int CSerialProtoWorkDA::Shutdown()
{
   initialized_ = false;
   return DEVICE_OK;
}

int CSerialProtoWorkDA::WriteToPort(unsigned long value)
{
   CSerialProtoWorkHub* hub = static_cast<CSerialProtoWorkHub*>(GetParentHub());
   if (!hub || !hub->IsPortAvailable())
      return ERR_NO_PORT_SET;
...
int CSerialProtoWorkDA::WriteSignal(double volts)
{
...
int CSerialProtoWorkDA::SetSignal(double volts)
{
   volts_ = volts;
   if (gateOpen_) {
      gatedVolts_ = volts_;
...
int CSerialProtoWorkDA::SetGateOpen(bool open)
{
   if (open) {
      gateOpen_ = true;
      gatedVolts_ = volts_;
...
int CSerialProtoWorkDA::OnVolts(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet)
   {
      // nothing to do, let the caller use cached property
...
int CSerialProtoWorkDA::OnMaxVolt(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet)
   {
      pProp->Set(maxV_);
...
int CSerialProtoWorkDA::OnChannel(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet)
   {
...
/*
 * SerialProtoWork input.  Can either be for all pins (0-6)
 * or for an individual pin only
 */

CSerialProtoWorkInput::CSerialProtoWorkInput() :
   mThread_(0),
   pin_(0),
   name_(g_DeviceNameSerialProtoWorkInput)
{
...
int CSerialProtoWorkInput::Shutdown()
{
   if (initialized_)
      delete(mThread_);
   initialized_ = false;
...
int CSerialProtoWorkInput::Initialize()
{
   CSerialProtoWorkHub* hub = static_cast<CSerialProtoWorkHub*>(GetParentHub());
   if (!hub || !hub->IsPortAvailable()) {
      return ERR_NO_PORT_SET;
   ...
   // Digital Input
   CPropertyAction* pAct = new CPropertyAction (this, &CSerialProtoWorkInput::OnDigitalInput);
   ...
   {
      CPropertyActionEx *pExAct = new CPropertyActionEx(this, &CSerialProtoWorkInput::OnAnalogInput, i);
      std::ostringstream os;
      os << "AnalogInput" << i;
   ...
   mThread_ = new SerialProtoWorkInputMonitorThread(*this);
   mThread_->Start();
...
void CSerialProtoWorkInput::GetName(char* name) const
{
  CDeviceUtils::CopyLimitedString(name, name_.c_str());
}

bool CSerialProtoWorkInput::Busy()
{
   return false;
}

int CSerialProtoWorkInput::GetDigitalInput(long* state)
{
   CSerialProtoWorkHub* hub = static_cast<CSerialProtoWorkHub*>(GetParentHub());
   if (!hub || !hub->IsPortAvailable())
      return ERR_NO_PORT_SET;
...
int CSerialProtoWorkInput::ReportStateChange(long newState)
{
   std::ostringstream os;
   os << newState;
...
int CSerialProtoWorkInput::OnDigitalInput(MM::PropertyBase*  pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet)
   {
      long state;
...
int CSerialProtoWorkInput::OnAnalogInput(MM::PropertyBase* pProp, MM::ActionType eAct, long  channel )
{
   CSerialProtoWorkHub* hub = static_cast<CSerialProtoWorkHub*>(GetParentHub());
   if (!hub || !hub->IsPortAvailable())
      return ERR_NO_PORT_SET;
...
int CSerialProtoWorkInput::SetPullUp(int pin, int state)
{
   CSerialProtoWorkHub* hub = static_cast<CSerialProtoWorkHub*>(GetParentHub());
   if (!hub || !hub->IsPortAvailable())
      return ERR_NO_PORT_SET;
...
int CSerialProtoWorkInput::ReadNBytes(CSerialProtoWorkHub* hub, unsigned int n, unsigned char* answer)
{
   MM::MMTime startTime = GetCurrentMMTime();
   unsigned long bytesRead = 0;
...
SerialProtoWorkInputMonitorThread::SerialProtoWorkInputMonitorThread(CSerialProtoWorkInput& aInput) :
   state_(0),
   aInput_(aInput)
{
}

SerialProtoWorkInputMonitorThread::~SerialProtoWorkInputMonitorThread()
{
   Stop();
   wait();
}

int SerialProtoWorkInputMonitorThread::svc() 
{
   while (!stop_)
   {
      long state;
...
void SerialProtoWorkInputMonitorThread::Start()
{
   stop_ = false;
   activate();
}
